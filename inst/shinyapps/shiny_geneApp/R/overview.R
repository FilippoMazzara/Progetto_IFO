         ...
         shiny::sidebarLayout(
           ...
           shiny::sidebarPanel(
             ...
             shiny::wellPanel(
               ...
               shiny::fileInput(ns("fromfile"), label=htmltools::HTML('Carica un file .tsv </label><span data-toggle="tooltip" style="float:right" data-placement="right" title="" data-original-title="A tooltip">
               ...
               shiny::selectInput(ns("fromserver"),"Seleziona il campione memorizzato", choices = c("paziente1","paziente2","paziente3"), selectize=TRUE,selected="paziente...")
             ),
             #anche questo panello puo essere reso interattivo e dinamico,compare diverso e compare solo per una certa condizione
             shiny::wellPanel(
               shiny::sliderInput(ns("bins"), "Number of bins:",min = 1,max = 50,value = 30)
             )
           ),
           ...
           shiny::mainPanel(
             shiny::tabsetPanel(
               shiny::tabPanel("pannello1",
                               shiny::fluidRow(shiny::tags$span(
...
homepanel_server <- function(id) {
  shiny::moduleServer(id,
               function(input, output, session) {
                 #bottone inutile
                 output$inutile <- shiny::renderUI(htmltools::HTML( '<button type="submit" disabled style="display: none" aria-hidden="true"></button>'))
                 #grafico
